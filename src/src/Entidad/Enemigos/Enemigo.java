...
public abstract class Enemigo extends Personaje {
	...
	public void actuar() {
		Iterator itAliados = mapa.getMapa().getAliados().iterator();
		Aliado aliado;
		boolean encontreColision = false;
		while (itAliados.hasNext() && encontreColision == false) {
			aliado = (Aliado) itAliados.next();
			if(aliado.getPosicionY()==this.getPosicionY() &&
					this.getPosicionX() - aliado.getPosicionX() <= 40 &&
		...
		Iterator itObjetos = mapa.getMapa().getObjetos().iterator();
		Objeto objeto;
		while (itObjetos.hasNext() && encontreColision == false) {
			objeto = (Objeto) itObjetos.next();
			if(objeto.getPosicionY()==this.getPosicionY() &&
					this.getPosicionX() - objeto.getPosicionX() <= 40 &&
	...
	public void setVida(int vida) {
		...
		if (vida <= 0) {
			...
			if(nro == 1) {Juego.getJuego(null).crearPowerMuerte(posicionX,posicionY);}
